...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Approximating the binomial distribution with a normal distribution"),
...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
      ...
      plot(NULL, xlim=c(min(xs), max(xs)), ylim=c(0, max(ys.binomial)), ylab="", xlab="", xaxt="n", bty="L",
           main = "")
      axis(1, at = xs)
      segments(xs, rep(0, n), xs, ys.binomial, lwd = 9, col = "gray")
    ...
    output$error <- renderText({
      ...
      paste0("Maximal relative error: ", round(error, 2), " (", label_percent(accuracy = 0.1)(error), ")")
    })
